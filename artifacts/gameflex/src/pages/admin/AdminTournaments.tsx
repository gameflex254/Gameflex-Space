...
export default function AdminTournaments() {
  ...
  return (
    <div>
        ...
        {tournaments.map((t: any) => (
          <div key={t.id} className="grid grid-cols-8 gap-4 p-4 border-t border-border/50 items-center text-sm">
            <div className="col-span-2 flex items-center gap-3">
              {t.image_url && <img loading="lazy" decoding="async" src={t.image_url} alt="" className="h-10 w-14 object-cover rounded" />}
      ...
      {/* Edit tournament */}
      <Dialog open={!!editing} onOpenChange={(open) => { if (!open) { setEditing(null); setEditImageFile(null); } }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            >
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <Input id="edit_title" value={editing.title} onChange={(e) => setEditing({ ...editing, title: e.target.value })} required />
                </div>
                <div className="col-span-2">
                ...
                <div>
                  ...
                  <Select value={editing.game} onValueChange={(v) => setEditing({ ...editing, game: v })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {['fifa', 'cod', 'pubg', 'fortnite', 'apex', 'valorant', 'other'].map(g => <SelectItem key={g} value={g}>{g.toUpperCase()}</SelectItem>)}
                    </SelectContent>
                ...
                <div>
                  ...
                  <Input id="edit_entry_fee" type="number" value={editing.entry_fee} onChange={(e) => setEditing({ ...editing, entry_fee: +e.target.value })} />
                </div>
                <div>
                  <Label htmlFor="edit_prize_pool">Prize Pool (KES)</Label>
                  <Input id="edit_prize_pool" type="number" value={editing.prize_pool} onChange={(e) => setEditing({ ...editing, prize_pool: +e.target.value })} />
                </div>
                <div>
                  <Label htmlFor="edit_max">Max Participants</Label>
                  <Input id="edit_max" type="number" value={editing.max_participants} onChange={(e) => setEditing({ ...editing, max_participants: +e.target.value })} />
                </div>
                <div>
                  <Label htmlFor="edit_start">Start Date</Label>
                  <Input id="edit_start" type="datetime-local" value={editing.start_date} onChange={(e) => setEditing({ ...editing, start_date: e.target.value })} required />
                </div>
                <div>
                ...
                <div className="col-span-2">
                  ...
                  <Textarea id="edit_description" value={editing.description} onChange={(e) => setEditing({ ...editing, description: e.target.value })} />
                </div>
                <div className="col-span-2">
                  <Label htmlFor="edit_rules">Rules</Label>
                  <Textarea id="edit_rules" value={editing.rules} onChange={(e) => setEditing({ ...editing, rules: e.target.value })} />
                </div>
              </div>
      ...
      <AlertDialog open={!!pendingDelete} onOpenChange={(open) => { if (!open) setPendingDelete(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              "{pendingDelete?.title}" and its registrations will be permanently removed. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
